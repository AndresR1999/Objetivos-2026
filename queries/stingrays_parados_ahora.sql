WITH ultimos AS (
    SELECT 
        ...
        REGEXP_SUBSTR(start_location_wcs, 'Level[0-9]+') AS LEVEL_ID,
        MAX(CAST(FINISHED_TIME AS DATE)) AS ULTIMO_FIN
    FROM VW_HOST_TRACE_MOVEMENTS@dbtgwp
    WHERE start_location_wcs LIKE '/Stingray/Aisle%/Level%'
      AND finishing_time IS NULL
    GROUP BY 
        REGEXP_SUBSTR(start_location_wcs, 'Aisle[0-9]+'),
        REGEXP_SUBSTR(start_location_wcs, 'Level[0-9]+')
),
movimientos_por_estado AS (
    SELECT 
        REGEXP_SUBSTR(origin, 'Aisle[0-9]+') AS AISLE,
        REGEXP_SUBSTR(origin, 'Level[0-9]+') AS LEVEL_ID,
    ...
    WHERE origin LIKE '/Stingray/Aisle%/Level%'
      AND status IN ('Created', 'SentToMFS', 'Executing')
    GROUP BY 
        REGEXP_SUBSTR(origin, 'Aisle[0-9]+'),
        REGEXP_SUBSTR(origin, 'Level[0-9]+')
)
SELECT 
    u.AISLE,
    u.LEVEL_ID,
